...
def parseContent(URL):
    ...
    if isLongText:
        try:
            ...
            print(str(e))
            print('Was expecting longtext but key does not exist,using original text as default')
...
class Tweet:
    ...
    def debug(self):
        print(f'Username:{self.user} Contains pic:{self.contain_pics()} IsRetweet:{self.is_retweet()}')
    ...
    def publish_to_tumblr(self):
        ...
        if (len(self.pics) != 0):
            ...
            TUMBLR_CLIENT.create_photo(blogName, state="published", tags=["weibo"],data=data,
                                       format="html",caption=tweet)
            ...
            try:
                with open('/tmp/'+'video.mp4','rb') as f:
                    ...
                    if r.status_code == 200 and shortcode is not None:
                        embed = session.get("https://api.streamable.com/oembed.json?url=https://streamable.com/"+shortcode)
                        if embed.status_code == 200:
                            print("Posting video to tumblr...")
                            html = embed.json().get("html","")
                            tweet_a_tag = f"""<a href="{"https://weibo.com/"+self.profileurl}">{self.user}:</a>"""
                            tweet = "<div>"+tweet_a_tag+self.originalText+"</div>"
                            TUMBLR_CLIENT.create_video(blogName, caption=tweet, format = "html",
                                    embed=html)
                        else:
        ...
        tweet_a_tag = f"""<a href="{"https://weibo.com/"+self.profileurl}">{self.user}:</a>"""
        tweet = "<div>"+tweet_a_tag+self.originalText+"</div>"
        TUMBLR_CLIENT.create_text(blogName, state="published", tags=["weibo"],format="html",body=tweet)
...
class Retweet(Tweet):
    ...
    def publish_to_tumblr(self):
        ...
        if (len(self.pics) != 0):
            ...
            TUMBLR_CLIENT.create_photo(blogName, state="published", tags=["testing", "ok"],data=data,
                                       format="html",caption=tweet+retweet)
            # Cleanup: Delete photos in /tmp folder
            for file in data:
                if os.path.exists(file):
                    os.remove(file)
                ...
                with open('/tmp/'+'video.mp4','rb') as f:
                    ...
                    if r.status_code == 200 and shortcode is not None:
                        ...
                        if embed.status_code == 200:
                            ...
                            retweet_a_tag = f"""<a href="{"https://weibo.com/"+self.retweetUserUrl}">{self.retweetUser}:</a>"""
                            retweet =  "<div>"+retweet_a_tag+self.retweetText+"</div>"
                            TUMBLR_CLIENT.create_video(blogName, caption=tweet+retweet, format = "html",
                                    embed=html)
                        else:
                            print("Error getting html embed code for video,exiting")
                            sys.exit(1)
        ...
        retweet_a_tag = f"""<a href="{"https://weibo.com/"+self.retweetUserUrl}">{self.retweetUser}:</a>"""
        retweet =  "<div>"+retweet_a_tag+self.retweetText+"</div>"
        TUMBLR_CLIENT.create_text(blogName, state="published", tags=["weibo"],format="html",body=tweet+retweet)
